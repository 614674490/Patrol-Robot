...
class Shared(object):
    ...
    bg = pygame.image.load(
        "/home/pi/Desktop/Patrol-Robot/RasPiCar_V1.6/PiCarMoudle/bg.jpg").convert()
    # 在窗口上绘制图片
    screen.blit(bg, (0, 0))
    # 更新图片
                ...
                for event in pygame.event.get():
                    if event.type == pygame.KEYDOWN:
                        if event.key == K_ESCAPE:
                            ...
                            data.set_FLAG('Automatic-Avoidance')
                            ...
                            data.set_FLAG('Automatic-Trackline')
...
class Thread_Motor (threading.Thread, Motor):
            ...
            while Shared.FLAG > 0:
                ...
                if Shared.FLAG == Shared.Mode['Manual']:
                    ...
                    if Shared.hc_sr04.dectect_block():
                        ...
                        while True:
                            count = count+1
                            if Shared.hc_sr04.dectect_block() and count > 45:  # 80 75 300
...
class Thread_Steer (threading.Thread, Steering):
...
class Thread_Dectect (threading.Thread, HC_SR04):

    def __init__(self, threadID):
        threading.Thread.__init__(self)
        self.threadID = threadID
        self._stop_event = threading.Event()
        # HC_SR04.__init__(self)
...
class AutoModeProcess(Process):
    ...
    def run(self):
        ...
        while data.get_FLAG() == Shared.Mode['Wait']:
            pass
        while data.get_FLAG() > 0:
            if data.get_FLAG() == Shared.Mode['Automatic-Trackline']:
                ret, frame = cap.read()
                # 灰度化
                ...
                '''
            '''
            elif data.get_FLAG() == Shared.Mode['Automatic-Avoidance']:
                if Shared.hc_sr04.dectect_block():
                    Shared.motor.ahead()
                else:
                    Shared.motor.stop()
